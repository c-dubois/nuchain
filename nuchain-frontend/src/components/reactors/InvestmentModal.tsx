...
interface InvestmentModalProps {
    ...
    isOpen: boolean;
    onClose: () => void;
    onInvest: (reactorId: number, amount: number) => Promise<{ tx_hash: string; tx_url: string }>;
}
...
export const InvestmentModal: React.FC<InvestmentModalProps> = ({
    ...
    const [txResult, setTxResult] = useState<{ tx_hash: string; tx_url: string } | null>(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const result = await onInvest(reactor.id, investmentAmount);
            setTxResult(result);
        } catch {
    ...
    const displayHash = txResult 
        ? (txResult.tx_hash.startsWith('0x') ? txResult.tx_hash : `0x${txResult.tx_hash}`)
        : '';
        ...
        <>
            ...
            <div className="investment-modal">
                <div className="modal-header">
                    <h2>{txResult ? 'Investment Complete' : `Invest in ${reactor.name}`}</h2>
                    ...
                    <>
                        <div className="modal-body">
                            <div className="investment-success">
                                ...
                                <p className="success-reactor">invested in {reactor.name}</p>
                                ...
                                <a
                                    href={txResult.tx_url}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="success-basescan-link"
                                >
                                    View on BaseScan ↗
                                </a>
                            </div>
